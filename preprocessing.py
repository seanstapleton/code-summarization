...
def get_path(terminal, pair_terminal, root):
    def helper(node, dest_node, path=''):
        node.visited = True
        if node == dest_node:
            return path
        path += node.name + '|'
        for child in node.children:
            if not child.visited:
                output_path = helper(child, dest_node, path=path)
                if output_path:
                    return output_path
        if not node.parent.visited:
            output_path = helper(node.parent, dest_node, path=path)
            if output_path:
                return output_path
        return False
...
def convert_code_to_ast_paths(code_string, path_len_limit=None):
    ...
    for i in range(len(terminals)-1):
        ...
        for j in range(i+1,len(terminals)):
            ...
            if path_len_limit and len(path.split('|')) > path_len_limit-2:
                continue
